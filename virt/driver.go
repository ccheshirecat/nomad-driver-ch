...
func (d *VirtDriverPlugin) SetConfig(cfg *base.Config) error {
	...
	v := cloudhypervisor.NewWithSkipValidation(d.baseCtx, d.logger, &d.config.CloudHypervisor, &d.config.Network, &d.config.VFIO, d.dataDir, true)
	d.virtualizer = v
	d.taskGetter = v
...
func (d *VirtDriverPlugin) StartTask(cfg *drivers.TaskConfig) (*drivers.TaskHandle, *drivers.DriverNetwork, error) {
	...
	if len(driverConfig.NetworkInterfacesConfig) == 0 {
		...
		for i, netInterface := range driverConfig.NetworkInterfacesConfig {
			if netInterface.Bridge != nil {
				...
				d.logger.Debug("found network interface without bridge", "index", i)
			}
		}
	}
	...
	dc := &domain.Config{
		...
		VFIODevices: driverConfig.VFIODevices,
	}
