...
func NewVFIOManager(logger hclog.Logger) VFIOManager {
	return &vfioManager{
		logger:     logger.Named("vfio-manager"),
		fileSystem: &realFileSystem{},
	}
}

// newVFIOManagerWithFS creates a VFIO manager with custom filesystem (for testing)
func newVFIOManagerWithFS(logger hclog.Logger, fs FileSystem) VFIOManager {
	return &vfioManager{
		logger:     logger,
		fileSystem: fs,
	}
